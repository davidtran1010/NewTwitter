...
class UserInfoAPI{
    ...
    return Promise{ fullfil, reject in
        ...
        twitter?.verifyCredentials(userSuccessBlock: { (userName, userID) in
            twitter?.getUsersShow(forUserID: userID, orScreenName: nil, includeEntities: nil, successBlock: { (user) in
                ...
                let bytes = try! JSONSerialization.data(withJSONObject: userDict, options: [])
                let jsonString = String(data: bytes,
                                        encoding: .ascii)
    ...
    static func fetchUserInfo(from session:TWTRSession)->Promise<(UserInfoResponse)>{
        return Promise{ fullfil, reject in
            let twitter = STTwitterAPI(oAuthConsumerKey: ApiInfo.consumerKey, consumerSecret: ApiInfo.consumerSecret, oauthToken: session.authToken, oauthTokenSecret: session .authTokenSecret)
            ...
            twitter?.verifyCredentials(userSuccessBlock: { (userName, userID) in
                twitter?.getUsersShow(forUserID: userID, orScreenName: nil, includeEntities: nil, successBlock: { (user) in
                    ...
                    print(jsonString!)
                    let response: UserInfoResponse = Mapper<UserInfoResponse>().map(JSONString: jsonString!)!
                    fullfil(response)
